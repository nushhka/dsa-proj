...
#include <string>
// #include <bits/stdc++.h>    
#include "TravelDesk.cpp"// Include your actual class header file here
using namespace std;
...
void test_add_trip() {
    ...
    // Add trips
    for (int i = 0; i < 10; ++i) {
        std::string vehicleNumber = "A" + to_string(i);
        travelDesk.add_trip(vehicleNumber, 4, "LocationA", "LocationB", 1000 + i * 100);
    }

    // Add trips for LocationX
    for (int i = 0; i < 10; ++i) {
        std::string vehicleNumber = "X" + to_string(i);
        travelDesk.add_trip(vehicleNumber, 3, "LocationX", "LocationY", 1500 + i * 100);
    }
    
    // Verify that the tree is a proper BST
    BinaryTreeNode* rootLocationA = Location("LocationA").getServicePtr("LocationA")->getBSTHead(); // Add a function to get root by location
    assert(isBST(rootLocationA));

    BinaryTreeNode* rootLocationX = Location("LocationX").getServicePtr("LocationX")->getBSTHead(); // Add a function to get root by location
